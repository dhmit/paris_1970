    ...
    def clean(self):
        ...
        if self.instance._state.adding is True:
            if slug is None:
                ...
                if BlogPost.objects.filter(slug=slug).exists():
                    raise forms.ValidationError(
                        {'slug': _('Autogenerated slug already exists. Enter unique '
                                 'slug')})

            if BlogPost.objects.filter(slug=slug).exists():
                raise forms.ValidationError(
                    {"slug": _("Slug already exists. Enter unique slug or leave empty")})
        else:
            if slug is None:
                slug = self.slug_generator()
            ...
            if BlogPost.objects.filter(slug=slug).exists() and slug != self.instance.slug:
                raise forms.ValidationError(
                    {"slug": _("Slug already exists. Enter unique slug")})
