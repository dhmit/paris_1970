"""
    This was the original syncdb command that we wrote in Summer 2020, to support
    a workflow that had all of our source images on Google Drive and metadata that we'd
    transcribed for those images in a Google Sheet.

    This script built your local development database from those sources, downloaded
    the appropriate source images so you could run analysis functions locally, etc.

    Our new workflow as of Fall 2022 doesn't involve any of this, so this very useful
    script is now a relic of the past.

    We're keeping it here for reference (instead of deleting it), as there's some 
    code here that we'll probably want to salvage in order to support a full rebuild 
    of our production database (e.g., regenerating all of the map squares).
