...
def analyze(photo: Photo):
    ...
    print(type(grayscale_image.shape))
    ...
    # One method: Find the standard deviation of each row of pixels
    # -------------
    # grayscale_image = list(cv2.cvtColor(image, cv2.COLOR_BGR2GRAY))
    # stdevs = []
    # for i, row in enumerate(grayscale_image):
    #     stdevs.append(float(np.std(row)))
    # Compute average of stdevs as above
